CDSafeHelpContentDB = {
    enUS = {
        button = "Help",
        title = "CDSafe author: Anaxir",
        body = "1.CDSafe only works in raid groups.\n"
            .. "2.Raid members can only see the leader's raid progress if the leader also has this addon installed.\n"
            .. "3.When you approach a raid entrance, the addon will automatically check for lockout risk.\n"
            .. "4.If the leader is saved to that raid and your progress is not the same, you will be warned not to enter.\n"
            .. "5.If you and the leader have the same raid instance ID, no warning will be shown.",
    },
    zhCN = {
        button = "说明",
        title = "CDSafe - 作者：安娜希尔",
        body = "1.CDSafe 只在团队中生效。\n"
            .. "2.团长安装了插件，团员才能看到团长的副本进度。\n"
            .. "3.靠近副本入口时，插件会自动检查是否有锁本风险。\n"
            .. "4.如果团长已锁定该副本，与你不同CD，会提醒不要进入。\n"
            .. "5.如果你和团长是同一个副本ID，则不会提醒。",
    },
}
